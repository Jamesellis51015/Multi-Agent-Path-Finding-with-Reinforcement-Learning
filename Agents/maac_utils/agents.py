...
from Agents.maac_utils.misc import hard_update, gumbel_softmax, onehot_from_logits
from Agents.maac_utils.policies import make_policy#DiscretePolicy
...
class AttentionAgent(object):
    ...
    def __init__(self, num_in_pol, num_out_pol, hidden_dim=64,
                 lr=0.01, onehot_dim=0, base_policy_type = 'mlp'):
        """
        Inputs:
            num_in_pol (int): number of dimensions for policy input
            num_out_pol (int): number of dimensions for policy output
        """
        DiscretePolicy = make_policy(base_policy_type)
