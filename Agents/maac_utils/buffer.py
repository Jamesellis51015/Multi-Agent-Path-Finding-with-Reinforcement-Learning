...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            hldr = [max_steps]
            hldr.extend(list(odim))
            odim = hldr
            self.obs_buffs.append(np.zeros(odim, dtype=np.float32))
            self.ac_buffs.append(np.zeros((max_steps, adim), dtype=np.float32))
            self.rew_buffs.append(np.zeros(max_steps, dtype=np.float32))
            self.next_obs_buffs.append(np.zeros(odim, dtype=np.float32))
    ...
    def push(self, observations, actions, rewards, next_observations, dones):
        ...
        for agent_i in range(self.num_agents):
            # hldr = observations[:, agent_i]
            # hldr2 = np.vstack(observations[:, agent_i])
           ...
           # print("a_buf {}".format(self.ac_buffs[agent_i][self.curr_i:self.curr_i + nentries]))
          #  print("a_rec {}".format(actions[agent_i]))
    ...
    def sample(self, N, to_gpu=False, norm_rews=True): #Changed norm_rews to False
        ...
        if norm_rews:
         ...
         #   hldr1 = [self.rew_buffs[i][:self.filled_i].mean() for i in range(self.num_agents)]
           # hldr2 = [self.rew_buffs[i][:self.filled_i].std() for i in range(self.num_agents)]
