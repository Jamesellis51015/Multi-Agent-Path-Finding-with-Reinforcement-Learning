...
from Agents.maac_utils.misc import soft_update, hard_update, enable_gradients, disable_gradients
from Agents.maac_utils.agents import AttentionAgent
from Agents.maac_utils.critics import AttentionCritic
...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
        ...
        """
        if "share_actor" in kwargs.keys():
            self.share_actor = kwargs["share_actor"]
        else:
            self.share_actor = False
        self.base_policy_type = kwargs["base_policy_type"]
        self.nagents = len(sa_size)

        if self.share_actor:
            params = agent_init_params[0] #Assume all inint params the same for shared agent actors
            pol = AttentionAgent(lr=pi_lr,
                                      hidden_dim=pol_hidden_dim, base_policy_type=self.base_policy_type,
                                      **params)
            self.agents = [pol for _ in range(len(agent_init_params))] #for each agent
        else:
    ...
    def init_from_env(cls, env, gamma=0.95, tau=0.01,
        ...
        if "share_actor" in kwargs.keys():
            share_actor = kwargs["share_actor"]
        else:
            share_actor = False
        agent_init_params = []
        sa_size = []
        for acsp, obsp in zip(env.action_space,
            ...
            agent_init_params.append({'num_in_pol': obsp.shape,
                                      'num_out_pol': acsp.n})
            sa_size.append((obsp.shape, acsp.n))
        ...
        init_dict = {'gamma': gamma, 'tau': tau,
                     ...
                     'base_policy_type': kwargs["base_policy_type"]}
