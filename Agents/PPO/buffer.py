...
class PPO_Buffer():
    ...
    def add(self, obs, reward, value, nobs,a_prob, a_select, info, dones, hc_act, hc_cr, hc_act_n, hc_cr_n, blocking):
        for i, (ob, r, v, nob, a_p, a_s, inf, don, hc_a, hc_c, hc_a_n, hc_c_n, block) in enumerate(zip(obs, reward, value, nobs, a_prob, a_select, info, dones, hc_act, hc_cr, hc_act_n, hc_cr_n, blocking)):
    ...
    def sample(self,gamma, lambda_, empty = True, blocking = False):
        ...
        hc_cr_hldr = []
        blk_hldr = []
        for agnt_ob, agnt_ap, agnt_as, agnt_adv, agnt_v, hc_actr, hc_cr, hc_actr_n, hc_cr_n, block \
        ...
        self.block_buff.values()):
            
            for roll_ob, roll_ap, roll_as, roll_adv, roll_v, roll_hc_a, roll_hc_c, roll_hc_n, roll_hc_n, roll_blk  \
            in zip(agnt_ob, agnt_ap, agnt_as, agnt_adv, agnt_v, hc_actr, hc_cr, hc_actr_n, hc_cr_n, block):
     ...
     #   adv = (adv - adv.mean())/(adv.std() + 1e-5)
       ## hldr2 = adv.mean()
        ...
        if len(obs_vec) != 0:
            obs = (obs, obs_vec)
        return obs, a_prob, a_select, adv, v, inf, hc_actr, hc_cr, blocking_labels, blocking_pred
