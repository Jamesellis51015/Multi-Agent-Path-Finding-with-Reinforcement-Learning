...
class Value_Net(nn.Module):
    """No replay buffer --multi-agent """
    def __init__(self, args, observation_space):
        self.args =  args
        super().__init__()
...
class AC_Separate(nn.Module):
    '''Reinforce with baseline seperate networks '''
    def __init__(self, args, observation_space, action_space):
        ...
        self.value_net = Value_Net(args, observation_space)
    ...
    def update(self, transitions_batch, per_agent = False):
        ...
        #TODO: Instead of doing as below, make one big batch, get loss and update.
        ...
        #NB:
        #NB TODO: Do not average by nr of agents, only by number of steps, in order to compare fairly with ic3net
        #NB:  
        total_action_loss = torch.tensor(0).double().to(config.device)
        total_value_loss = torch.tensor(0).double().to(config.device)
        ...
        return total_value_loss.item(),total_action_loss.item()
    ...
    def take_action(self, observations):
        ...
        for agent_id, obs in observations.items():
            ...
            a_log_prob = self.forward(obs)
            val = self.value_net.forward(obs)
    ...
    def _process_obs(self, obs):
        ...
        obs = torch.tensor(obs).unsqueeze(0).to(config.device)
        return obs
